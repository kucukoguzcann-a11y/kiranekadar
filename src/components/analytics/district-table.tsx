...
type SortField = 'name' | 'count' | 'minRent' | 'medianRent' | 'maxRent' | 'rentPerSqm';
type SortDirection = 'asc' | 'desc';

export default function DistrictTable({ districts, citySlug }: DistrictTableProps) {
  ...
  const processedDistricts = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter((d) => d.name.toLocaleLowerCase('tr-TR').includes(query));
    }

    // 2. Sort
    result.sort((a, b) => {
      // Keep districts with 0 data at the bottom unless sorting by name
      if (sortField !== 'name') {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="hidden md:grid grid-cols-[2.5fr_1fr_1.2fr_1.2fr_1.2fr_1.2fr_110px] gap-4 px-6 py-3 border-b border-gray-100 bg-gray-50 items-center">
          {headers.map((h) => {
        ...
        {processedDistricts.length > 0 ? (
          processedDistricts.map((d, idx) => (
            <div
              key={d.id}
              className="flex flex-col md:grid md:grid-cols-[2.5fr_1fr_1.2fr_1.2fr_1.2fr_1.2fr_110px] gap-2 md:gap-4 px-6 py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors items-start md:items-center"
            >
              ...
              <div className="flex items-center justify-between w-full md:w-auto min-w-0">
                ...
                <div className="md:hidden text-right">
                  {d.medianRent > 0 ? (
                    <div className="font-extrabold text-sm text-emerald-700">{formatCurrency(d.medianRent)}</div>
                  ) : (
                    <div className="text-gray-300 text-xs font-semibold">Veri yok</div>
                  )}
                  {d.rentPerSqm > 0 && (
                    <div className="text-[9px] text-gray-400">m²: {d.rentPerSqm} ₺</div>
                  )}
                </div>
              </div>
              ...
              <div className="hidden md:block text-sm text-gray-600 font-medium">
                {d.count > 0 ? d.count : <span className="text-gray-300">—</span>}
              </div>

              {/* Min Kira (Desktop) */}
              <div className="hidden md:block text-sm text-gray-600">
                {d.minRent > 0 ? formatCurrency(d.minRent) : <span className="text-gray-300">—</span>}
              </div>
              ...
              {/* Maks Kira (Desktop) */}
              <div className="hidden md:block text-sm text-gray-600">
                {d.maxRent > 0 ? formatCurrency(d.maxRent) : <span className="text-gray-300">—</span>}
              </div>

              {/* m² Fiyatı (Desktop) */}
              <div className="hidden md:block text-sm text-gray-600">
                {d.rentPerSqm > 0 ? `${d.rentPerSqm.toLocaleString('tr-TR')} ₺` : <span className="text-gray-300">—</span>}
              </div>

              {/* Action Button */}
              <div className="w-full md:w-auto flex justify-end pt-2 md:pt-0 shrink-0">
                <Link
                  href={`/${citySlug}-${d.slug}-kira-fiyatlari`}
                  aria-label={`${d.name} ilçesi kira analizini incele`}
          ...
          <div className="text-center py-8 text-sm text-gray-400">Aramanıza uygun ilçe bulunamadı.</div>
        )}
      </div>
    </div>
  );
